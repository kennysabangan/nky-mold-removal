...
function buildEmailHtml(data: {
  ...
  const pathMatch = pagePath.match(/\/mold-removal\/([^/]+)/);
  ...
  const sourceLabel = isHomePage ? 'Homepage' : `${cityDisplay} — Mold Remediation`;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const html = buildEmailHtml({
    siteName: site_name || 'Mold Remediation',
    fullName,
    email,
    phone: phone || '',
    comment: comment || '',
  ...
  try {
    await resend.emails.send({
      from: 'NKY Mold Removal Alerts <alerts@mursenmaintenance.com>',
